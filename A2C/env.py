...
class market_dynamic:
    ...
    def step(self,arriving_seg,assortment):#assortment是torch.zeros([env.batch_size, self.num_products],dtype=torch.int)
        #考虑batch,生成input
        pre = time.time()
        multiplier = (np.hstack((assortment, np.ones((self.batch_size, 1))))).reshape(self.batch_size, 58, 1)#加上不选
        multiplier = np.concatenate((np.repeat(multiplier, 6, axis=2), np.ones((self.batch_size, 58, 4))), axis=2)
        ...
        #关键语句
        with torch.no_grad():
            prob = self.Resnet(input_1, ass_onehot)#0.9几的概率不选
        #breakpoint()
        #初始化
