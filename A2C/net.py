...
class A2C(nn.Module):
    ...
    def forward(self, x):
        p_e = self.product_encoder(torch.cat((
            self.inv_encoder(x[:,:self.num_products]),
            self.price_encoder(x[:,self.num_products+len(self.cus_type):])), dim=1))
        c_e = self.cus_encoder(x[:,self.num_products:self.num_products+len(self.cus_type)])
    ...
    def roll_out(self, env, input_sequence, i):
        ...
        for num_step in range(self.args.num_steps):
            ...
            s = np.hstack((env.inventory_level, cus_type))#/ env.total_inv
    ...
    def _select_node(self, env, score, mask):
        ...
        if self.decode_type == "greedy":
            ...
            selected = p.exp().multinomial(self.cardinality,replacement=True)#无放回的抽样
            ass.scatter_(1,selected,1)
            ass = ass*torch.logical_not(mask)#注意：不是mask着的东西才能摆
