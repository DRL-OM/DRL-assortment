...
def create_data(args):
    ...
    X = X_0[:-1]
    Z = np.eye(4)
    prod_dup = np.repeat(X.reshape(1,-1),4,axis = 0).reshape(57*4,-1)
    prod_dup0 = np.repeat(X_0.reshape(1, -1), 4, axis=0).reshape(58*4, -1)
    cus_dup = np.repeat(Z,57,axis = 0)
    cus_dup0 = np.repeat(Z,58,axis=0)
    return X,Z,np.concatenate((prod_dup,cus_dup),axis=1).reshape(4,57,-1),\
           np.concatenate((prod_dup0,cus_dup0),axis=1).reshape(4,58,-1)
