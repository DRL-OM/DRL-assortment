...
def create_data(args):
    ...
    Z = np.load(args.c_file)
    X = X_0[:-1,:]
    prod_dup = np.repeat(X.reshape(1, -1), args.num_cus_types, axis=0).reshape(args.num_products* args.num_cus_types, -1)
    prod_dup0 = np.repeat(X_0.reshape(1, -1), args.num_cus_types, axis=0).reshape((args.num_products+1) * args.num_cus_types, -1)
    cus_dup = np.repeat(Z, args.num_products, axis=0)
    cus_dup0 = np.repeat(Z, args.num_products+1, axis=0)
    #第三个输出用于other agents
    #第四个输出用于env
    return X, Z, np.concatenate((prod_dup,cus_dup), axis=1).reshape(args.num_cus_types, args.num_products, -1), \
           np.concatenate((prod_dup0,cus_dup0), axis=1).reshape(args.num_cus_types, args.num_products+1, -1)
...
def generate_set(args,T,seed_,seg_prob):
    ...
    for i in range(args.test_episode):
        input_sequence = np.random.choice \
                (a=np.arange(args.num_cus_types), size=T+20, replace=True, p=seg_prob)
