...
from other_agents import OA_agent,myopic_agent,E_IB_agent,optmyopic_agent,optEIB_agent
import torch,math
from numpy import *
...
def test(test_sequences,MNL_para,initial_inventory,products_price,args,logger,load=True,plot= False):
    ...
    for seed_ in range(args.seed_range):
        ...
        for seq_index in seq_indexlist:
            ...
            if not plot:
                if args.detail:
                    change_of_invOA,change_of_ROA,change_of_invM,change_of_RM,change_of_invE,change_of_RE = other_agents(OA_seller, myopic_seller, E_IB_seller,initial_inventory, T, products_price, input_sequence,args,logger,args.detail)
                else:
